...
def calculate_score(img_gt, img_test):
    ...
    ssim_score = ssim(img_gt, img_test, channel_axis=-1, data_range=1, win_size=11)

    gt_tensor = torch.from_numpy(img_gt).permute(2, 0, 1).unsqueeze(0).to(device)
    test_tensor = torch.from_numpy(img_test).permute(2, 0, 1).unsqueeze(0).to(device)

    with torch.no_grad():
        lpips_dis = lpips_vgg(gt_tensor, test_tensor, normalize=True)
